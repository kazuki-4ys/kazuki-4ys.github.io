...
function miiFileRead(buf) {
	...
	if (buf.length === CTR_MII_FILE_SIZE) {
		...
		rawData = null;
		tmpU16 = bufToUint16(buf, 0, false);
		editMii.isGirl = getBoolean((tmpU16 >>> 14) & 1);
		editMii.month = (tmpU16 >>> 10) & 0xF;
		editMii.day = (tmpU16 >>> 5) & 0x1F;
		editMii.favColor = (tmpU16 >>> 1) & 0xF;
		editMii.isFavorite = getBoolean(tmpU16 & 1);
		editMii.version = 1;
		editMii.name = bufToUTF16String(buf, 2, MII_NAME_LENGTH, false);
		editMii.height = buf[0x16];
		editMii.weight = buf[0x17];
		for (var i = 0; i < MII_ID_LENGTH; i++) {
			editMii.miiID[i] = buf[0x18 + i];
		}
		tmpU16 = bufToUint16(buf, 0x20, false);
		editMii.faceShape = (tmpU16 >>> 13);
		editMii.skinColor = (tmpU16 >>> 10) & 7;
		var facialFeature = (tmpU16 >>> 6) & 0xF;
		editMii.makeup = MAKEUP[facialFeature];
		editMii.wrinkles = WRINKLES[facialFeature];
		editMii.sharing = !getBoolean((tmpU16 >>> 2) & 1);
		tmpU16 = bufToUint16(buf, 0x22, false);
		editMii.hairStyle = tmpU16 >>> 9;
		editMii.hairColor = (tmpU16 >>> 6) & 7;
		editMii.flipHair = (tmpU16 >>> 5) & 1;
		tmpU32 = bufToUint32(buf, 0x24, false);
		editMii.eyebrowStyle = tmpU32 >>> 27;
		editMii.eyebrowRotation = (tmpU32 >>> 22) & 0xF;
		editMii.eyebrowColor = (tmpU32 >>> 13) & 7;
		editMii.eyebrowScale = (tmpU32 >>> 9) & 0xF;
		editMii.eyebrowYscale = 3;
		editMii.eyebrowYposition = (tmpU32 >>> 4) & 0x1F;
		editMii.eyebrowXspacing = tmpU32 & 0xF;
		tmpU32 = bufToUint32(buf, 0x28, false);
		editMii.eyeStyle = tmpU32 >>> 26;
		editMii.eyeRotation = (tmpU32 >>> 21) & 7;
		editMii.eyeYposition = (tmpU32 >>> 16) & 0x1F;
		editMii.eyeColor = (tmpU32 >>> 13) & 7;
		editMii.eyeScale = (tmpU32 >>> 9) & 7;
		editMii.eyeYscale = 3;
		editMii.eyeXspacing = (tmpU32 >>> 5) & 0xF;
		tmpU16 = bufToUint16(buf, 0x2C, false);
		editMii.noseStyle = tmpU16 >>> 12;
		editMii.noseScale = (tmpU16 >>> 8) & 0xF;
		editMii.noseYposition = (tmpU16 >>> 3) & 0x1F;
		tmpU16 = bufToUint16(buf, 0x2E, false);
		editMii.mouseStyle = tmpU16 >>> 11;
		editMii.mouseColor = (tmpU16 >>> 9) & 3;
		editMii.mouseScale = (tmpU16 >>> 5) & 0xF;
		editMii.mouseYscale = 3;
		editMii.mouseYposition = tmpU16 & 0x1F;
		tmpU16 = bufToUint16(buf, 0x30, false);
		editMii.glassesStyle = tmpU16 >>> 12;
		editMii.glassesColor = (tmpU16 >>> 9) & 7;
		editMii.glassesScale = (tmpU16 >>> 5) & 7;
		editMii.glassesYposition = tmpU16 & 0x1F;
		tmpU16 = bufToUint16(buf, 0x32, false);
		editMii.mustacheStyle = tmpU16 >>> 14;
		editMii.beardStyle = (tmpU16 >>> 12) & 3;
		editMii.beardColor = (tmpU16 >>> 9) & 7;
		editMii.mustacheScale = (tmpU16 >>> 5) & 0xF;
		editMii.mustacheYposition = tmpU16 & 0x1F;
		tmpU16 = bufToUint16(buf, 0x34, false);
		editMii.enableMole = tmpU16 >>> 15;
		editMii.moleScale = (tmpU16 >>> 11) & 0xF;
		editMii.moleYposition = (tmpU16 >>> 6) & 0x1F;
		editMii.moleXposition = (tmpU16 >>> 1) & 0x1F;
		editMii.creatorName = bufToUTF16String(buf, 0x36, MII_NAME_LENGTH, false);
	}
...
function miiEncode() {
	var i, tmpU32, tmpU16;
	var buf = new Uint8Array(CTR_MII_FILE_SIZE);
