...
function scanQrFromCanvas(){
    ...
    var code = scanCodeFromCanvas(cameraCanvas);
    if(code){
        if(code.binaryData.length === 0x70 || code.binaryData.length === 324 || code.binaryData.length === 372){
...
function setCamera(useFront){
    ...
    .catch( (err) => {
        if(err.name === 'NotAllowedError'){
            alert('カメラを使ってQRコードをスキャンするにはカメラの使用を許可してください。\nPermission denied.');
...
function stopCamerStrem(cameraStream){
...
QRFileReader.addEventListener('load',function(event){
    ...
    image.addEventListener('load',function(event){
        var qrResult = scanCodeFromLoadedImage(event.target);
        ...
        if(qrResult.binaryData.length !== 0x70 && qrResult.binaryData.length !== 324 && qrResult.binaryData.length !== 372){
            alert('これはMiiのQRコードではありません。\nThis isn\'t Mii QR code');
            return;
        }
