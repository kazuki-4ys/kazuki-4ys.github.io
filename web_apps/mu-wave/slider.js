...
class TestPlaySlider{
    constructor(_maxValue, _sliderDiv, _curPosKnobDiv, _loopStartKnobDiv, _loopEndKnobDiv){
        this.sliderBar = new SliderBar(_maxValue, _sliderDiv);
        this.curPosKnob = new SliderKnob(this.sliderBar, _curPosKnobDiv);
        this.loopStartKnob = new SliderKnob(this.sliderBar, _loopStartKnobDiv);
        this.loopEndKnob = new SliderKnob(this.sliderBar, _loopEndKnobDiv);
        window.requestAnimationFrame(this.animationFrameHandler.bind(null, this));
        this.loopFrontBar = this.sliderBar.sliderDiv.querySelector(".loopFrontBar");
        this.loopGapBar = this.sliderBar.sliderDiv.querySelector(".loopGapBar");
    }
    ...
    update(){
        var loopFrontBarLength = this.loopStartKnob.value / this.sliderBar.maxValue;
        if(loopFrontBarLength < 0)loopFrontBarLength = 0;
        if(loopFrontBarLength > 1)loopFrontBarLength = 1;
        var loopGapBarLength = (this.loopEndKnob.value / this.sliderBar.maxValue) - loopFrontBarLength;
        if(loopGapBarLength < 0)loopGapBarLength = 0;
        if(loopGapBarLength > 1)loopGapBarLength = 1;
        this.loopFrontBar.style.width = (100 * loopFrontBarLength) + "%";
        this.loopGapBar.style.width = (100 * loopGapBarLength) + "%";
    }
}
